...
import com.ujanglukmanbdg.githubtoday.adapter.SectionsPagerAdapterAboutMe
...
class AboutMeActivity : AppCompatActivity() {
    ...
    private var favoriteDatabase: ItemsItem? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        supportActionBar?.apply {
            ...
            title = resources.getString(R.string.about_my_profile)
        }

        val user = intent.getParcelableExtra<ItemsItem>(DetailSearchUserActivity.EXTRA_DETAIL_USERNAME)

        initViewModel()

        viewModel.getUserResult(getString(R.string.my_account)).observe(this) {
            when (it.status) {
        ...
        if (favoriteDatabase != null) {
            viewModel.getFavoriteUsername(getString(R.string.my_account)).observe(this){
                if (it.isNotEmpty()) {
                    isFavorite = true
                    buttonChecked()
                }
    ...
    private fun loadData(data: ResponseDetailUser) {
        ...
        binding.apply {
            ...
            val sectionsPagerAdapter = SectionsPagerAdapterAboutMe(this@AboutMeActivity)
