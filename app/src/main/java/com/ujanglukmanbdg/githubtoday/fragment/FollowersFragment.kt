...
import com.ujanglukmanbdg.githubtoday.databinding.FragmentFollowersBinding
...
class FollowersFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val user = activity?.intent?.getParcelableExtra<ItemsItem>(DetailSearchUserActivity.EXTRA_DETAIL_USERNAME)
    ...
    private fun initRecyclerView() {
        ...
        adapter.setOnItemClickCallback(object : ListSearchUserAdapter.OnItemClickCallback{
            override fun onItemClicked(user: ItemsItem) {
                DetailSearchUserActivity.start(activity, user)
            }
        })
    }
    ...
    private fun setFollowList(index: Int, user: ItemsItem) {
        when (index) {
            1 -> viewModel.getFollowers(user.login).observe(viewLifecycleOwner){ loadDataFollow(it)}
            2 -> viewModel.getFollowing(user.login).observe(viewLifecycleOwner){ loadDataFollow(it)}
        }
